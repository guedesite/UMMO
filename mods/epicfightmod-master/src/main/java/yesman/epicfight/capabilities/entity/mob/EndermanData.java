...
public class EndermanData extends BipedMobData<EndermanEntity> {
	...
	protected void initAttributes() {
		...
		this.orgEntity.getAttribute(EpicFightAttributes.IMPACT.get()).setBaseValue(1.8F);
	}
	...
	protected void initAI() {
		super.initAI();
		this.normalAttack1 = new AttackPatternPercentGoal(this, this.orgEntity, 0.0D, 1.23D, 0.4F, true, AttackCombos.ENDERMAN_KNEE);
		this.normalAttack2 = new AttackPatternPercentGoal(this, this.orgEntity, 0.0D, 1.9D, 0.4F, true, AttackCombos.ENDERMAN_KICK_COMBO);
		this.normalAttack3 = new AttackPatternPercentGoal(this, this.orgEntity, 3.0D, 4.0D, 0.1F, true, AttackCombos.ENDERMAN_SPINKICK);
		this.normalAttack4 = new AttackPatternPercentGoal(this, this.orgEntity, 0.0D, 2.0D, 0.2F, true, AttackCombos.ENDERMAN_JUMPKICK);
	...
	protected void convertRage() {
		...
		if (this.isRemote()) {
			...
			if (!this.orgEntity.isAIDisabled()) {
				...
				if(this.rageChase == null) {
					this.rageChase = new AIEndermanRush(this, this.orgEntity);
				}
				if(this.rageTarget == null) {
					this.rageTarget = new NearestAttackableTargetGoal<>(this.orgEntity, PlayerEntity.class, true);
				}
		...
		public void tick() {
			...
			if (this.delayCounter-- < 0 && !this.mobdata.getEntityState().isInaction()) {
				...
				if (flag) {
		        	...
		        	ModNetworkManager.sendToAllPlayerTrackingThisEntity(new STCPlayAnimationTP(kickAnimation.getNamespaceId(), kickAnimation.getId(), this.attacker.getEntityId(), 0.0F, 
	...
	static class AIEndermanRush extends AttackPatternGoal {
		...
		@Override
		public void startExecuting() {
			ModNetworkManager.sendToAllPlayerTrackingThisEntity(new STCPlayAnimationTarget(this.rushAnimation.getNamespaceId(), this.rushAnimation.getId(),
					this.attacker.getEntityId(), 0.0F, this.attacker.getAttackTarget().getEntityId()), this.attacker);
			this.accelator = 0.0F;
		}
		...
		@Override
		public void tick() {
			if (this.isTargetInRange(attacker.getAttackTarget()) && canExecuteAttack()) {
				this.mobdata.playAnimationSynchronize(Animations.ENDERMAN_GRASP, 0.0F);
			}
